document.addEventListener('DOMContentLoaded', () => {
    ...
    function showDetail({title, poster_path, overview}) {
        modalBoxContainer.innerHTML = ` <div id="modal-content" class="w-[80%] xl:max-w-3xl bg-neutral-800 mx-auto rounded-sm relative p-4 transform -translate-y-full opacity-0 transition-all duration-500 ease-out md:flex">
                                            ...
                                            <img src="${API_IMAGE_URL}${poster_path}" alt="Gambar" class="mx-auto w-40 md:mr-4">
                                            <div class="info-detail">
                                                <h3 class="text-white text-center mt-2">${title}</h3>
                                                <p class="text-gray-400">Release : </p>
                                                <p class="text-gray-400">Overview : ${overview}</p>
                                                <p class="text-gray-400">Popularity : </p>
                                            </div>
                                        </div>`
    ...
    moviesContainer.addEventListener('click', (e) => {
        if(e.target && e.target.id === 'btn-show-detail') {
            ...
            const overview = card.querySelector('p').textContent;
            const imgSrc = card.querySelector('img').getAttribute('src');
            const poster_path = imgSrc.replace('https://image.tmdb.org/t/p/w1280', '');

            showDetail({title, poster_path, overview});
        }
    })
